...
def add_to_cart(request, product_id):
    ...
    if product.variations.exists():
        q = Q(product=product)
        if color_name:
            q &= Q(color__name=color_name)
        else:
            q &= Q(color__isnull=True)
        if size_name:
            q &= Q(size__name=size_name)
        else:
    ...
    if variation:
        if variation.stock < 1:
            msg = f'Sorry, this variant is out of stock.'
            if is_ajax:
                return JsonResponse({'status': 'error', 'message': msg})
            messages.error(request, msg)
            return redirect('product_detail', slug=product.slug)
    elif product.variations.exists():
        msg = 'Please select a valid color and/or size.'
        if is_ajax:
            return JsonResponse({'status': 'error', 'message': msg})
        messages.error(request, msg)
        return redirect('product_detail', slug=product.slug)
    ...
    if request.method == 'POST':
        ...
        for item in items:
            ...
            if stock_available < item.quantity:
                messages.error(request, f'Sorry, "{item.product.name}" variant is out of stock.')
                return redirect('cart')
