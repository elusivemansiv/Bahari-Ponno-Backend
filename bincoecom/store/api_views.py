...
from .serializers import (
    ProductSerializer, CategorySerializer, ColorSerializer, SizeSerializer,
    CartSerializer, CartItemSerializer, OrderSerializer, HomeSliderSerializer, 
    PromotionCardSerializer, ShippingConfigSerializer, SellerOrderItemSerializer
)
...
class SellerStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        for i in range(6, -1, -1):
            ...
            daily_total = seller_items.filter(order__status='delivered', order__created_at__date=day).aggregate(total=Sum(F('price') * F('quantity')))['total'] or 0
            earnings.append(float(daily_total))
...
class SellerOrderViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def update_status(self, request, pk=None):
        ...
        # Simple status transition for proof of concept
        order = item.order
        if new_status in ['pending', 'processing', 'shipped', 'delivered', 'cancelled', 'return_approved']:
            # Validation: only allow return_approved if currently return_requested
            if new_status == 'return_approved' and order.status != 'return_requested':
                return Response({'error': 'Return can only be approved if it was requested'}, status=status.HTTP_400_BAD_REQUEST)
                
            order.status = new_status
            order.save()
            return Response({'status': 'Order status updated to ' + new_status})
