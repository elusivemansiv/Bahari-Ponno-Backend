...
class Product(models.Model):
    ...
    sizes = models.ManyToManyField(Size, blank=True, verbose_name='Quantities')
...
class ProductVariation(models.Model):
    ...
    size = models.ForeignKey(Size, on_delete=models.CASCADE, null=True, blank=True, verbose_name='Quantity')
    price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Override price for this variation")
    ...
    def __str__(self):
        ...
        qty_name = self.size.name if self.size else "N/A"
        return f"{self.product.name} - {color_name} - {qty_name}"
...
class CartItem(models.Model):
    ...
    def effective_price(self):
        if self.product:
            ...
            q = Q(product=self.product)
            if self.color:
                q &= Q(color__name=self.color)
            else:
                q &= Q(color__isnull=True)
            if self.size:
                q &= Q(size__name=self.size)
            else:
                q &= Q(size__isnull=True)
            variation = self.product.variations.filter(q).first()
